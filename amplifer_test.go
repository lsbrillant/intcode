...
func TestAmplifierOptimization(t *testing.T) {
	...
	for i, testcase := range testcases {
		_, result, err := OptimizeAmplifierPhaseSettings(testcase.program)
		if err != nil {
			t.Error(err)
		}
		if result != testcase.expected {
			t.Errorf("TestCase %d: expected=%d result=%d",
				i, testcase.expected, result)
		}
	}
}

func TestLoopedAmplifierOptimization(t *testing.T) {
	var testcases = [...]struct {
		program  string
		expected int
	}{
	...
	for i, testcase := range testcases {
		_, result, err := OptimizeLoopedAmplifierPhaseSettings(testcase.program)
		if err != nil {
			t.Error(err)
		}
		if result != testcase.expected {
